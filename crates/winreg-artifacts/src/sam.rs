...
//! - `SAM\Domains\Account\Users\<RID_hex>\V`      — binary user data
...
// ── Public API (stub — RED phase) ─────────────────────────────────────────────

/// Parse local user accounts from a SAM hive.
///
/// Walks `SAM\Domains\Account\Users\Names` for usernames, reads the `F` value
/// from the corresponding `Users\<RID>` key for timestamps and flags.
